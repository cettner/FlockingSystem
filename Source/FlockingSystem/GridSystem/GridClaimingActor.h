...
UCLASS()
class AGridClaimingActor : public AGridAttatchmentActor
{
	GENERATED_BODY()
...
protected:
	...
	virtual void PreTileChange(FGridTileData NewTile);
	virtual void PostTileChange(FGridTileData NewTile, FGridTileData PrevTile = FGridTileData());
	
public:
	virtual bool SetTileLocation(FGridTileData Moveto) override;
protected:
...
public:
	virtual TArray<UGridModifierType *> GetActiveModifiers(FGridTileData TileData);
	virtual TArray<FGridTileOffset> GetRelativeClaimSpace() const;
	virtual void SetGridClaimSpace(TArray<FGridTileData> ClaimedTiles, ASquareGameGrid * OwningGrid);
	virtual TArray<FGridTileData> GetGridClaimSpace() const;
...
protected:
    TArray<FGridTileData> GridClaimSpace;
