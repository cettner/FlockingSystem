...
bool AClaimableSquareGameGrid::AddGridActor(AGridClaimingActor * InActor, FGridTileData TileLocation, bool TickOnAdd)
{
...
bool AClaimableSquareGameGrid::RemoveGridActor(AGridClaimingActor * InActor, bool TickOnRemoval)
{
	...
	{
		InActor->SetGridClaimSpace(TArray<FGridTileData>(), nullptr);
	}

	return (success);
}

bool AClaimableSquareGameGrid::MoveGridActor(AGridClaimingActor * InActor, FGridTileData TileLocation)
{
	bool success = false;
	...
	TArray<FGridTileData> outtiles = TArray<FGridTileData>();
...
bool AClaimableSquareGameGrid::CanMoveTo(AGridClaimingActor * InActor, FGridTileData TileLocation)
{
	if (!InActor) return false;
	bool success = true;
...
TArray<UGridModifierType *> AClaimableSquareGameGrid::GetActiveModifiers(FGridTileData TileLocation) const
{
...
bool AClaimableSquareGameGrid::ApplyModifier(UGridModifierType* Modifier, FGridTileData TileLocation,  AGridClaimingActor * Source)
{
	bool success = false;
	int32 gridid = GetUniqueGridID(TileLocation);
...
bool AClaimableSquareGameGrid::ApplyModifier(UGridModifierType* Modifier, TArray<FGridTileData> TileLocations,  AGridClaimingActor * Source)
{
	bool success = true;
...
bool AClaimableSquareGameGrid::RemoveModifier(UGridModifierType* Modifier, FGridTileData TileLocation,  AGridClaimingActor * Source)
{
	bool success = false;
	int32 gridid = GetUniqueGridID(TileLocation);
...
bool AClaimableSquareGameGrid::RemoveModifier(UGridModifierType* Modifier, TArray<FGridTileData> TileLocations, AGridClaimingActor* Source)
{
	bool success = true;
	for (int i = 0; i < TileLocations.Num(); i++)
	{
...
bool AClaimableSquareGameGrid::GetGridTilesFromOffset(FGridTileData StartTile, TArray<FGridTileOffset> Offsets, TArray<FGridTileData>& OutTiles, bool bisstartinclusive) const
{
	...
	{
		FGridTileData newtile = FGridTileData();
